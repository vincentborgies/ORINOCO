...
document.getElementById('firstName').addEventListener('invalid', event => {
  event.target.setCustomValidity('Veuiller renseigner un format correct ex: Jean');
});

document.getElementById('lastName').addEventListener('invalid', event => {
  event.target.setCustomValidity('Veuillez renseigner un format correct ex: Dupont');
});

document.getElementById('adress').addEventListener('invalid', event => {
  event.target.setCustomValidity('Veuillez renseigner une adresse postable correcte ex: 17 rue de la paix');
});

document.getElementById('zipcode').addEventListener('invalid', event => {
  event.target.setCustomValidity('Veuillez renseigner un code postal valide ex: 75000');
});

document.getElementById('email').addEventListener('invalid', event => {
  event.target.setCustomValidity('veuillez renseigner une adresse e-mail valide ex: francoisdupont@hotmail.fr');
});

document.getElementById('city').addEventListener('invalid', event => {
  event.target.setCustomValidity('Veuillez renseigner un nom de ville correct ex: Paris');
});

// Création d'une fonction pour envoyer les informations au serveur et envoyer le client sur la page confirmation
...
const sendOrder = async () => {
  ...
  const response = await fetch('https://ab-p5-api.herokuapp.com/api/teddies/order', requestOptions);
...
confirmPurchase.addEventListener('click', e => {
  e.preventDefault();
  if (document.getElementById('purchase-form').reportValidity()) {
    sendOrder();
  }
});
